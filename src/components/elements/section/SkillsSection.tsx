...
export default function SkillsSection() {
  ...
  return (
    <>
      <div
        ref={sectionRef}
        className={`${!zoom ? "flex flex-wrap flex-col p-4 h-full w-full overflow-hidden" : "flex flex-wrap items-center justify-around h-full w-full overflow-y-auto"} max-sm:hidden`}
      >
      ...
      <div className="flex flex-col sm:hidden py-5 justify-evenly items-center space-y-3">
        <h1 className="text-2xl font-bold text-[#ED816C]">Mes compétences</h1>
        <div>
          <img src={ImgLang} alt=" Programming languages" />
        </div>
        <div>
          <img src={ImgTech} alt=" Technologies" />
        </div>
        <div>
          <img src={ImgProg} alt=" Programming" />
        </div>
        <div>
          <img src={ImgSoft} alt=" Soft Skills" />
        </div>
      </div>
    </>
  );
