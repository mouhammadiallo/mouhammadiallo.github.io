...
const HomePage = () => {
  ...
  const developer = {
    name: 'Mouhammad DIALLO',
    desc: 'Développeur web Full Stack',
    bio: 'The web is my puzzle...'
  };

  useEffect(() => {
    document.title = \`\${developer} | Portfolio\`;
    console.log('As Salamu Aleykum, Dunya!');
  }, []);
...
export default HomePage;`;
  ...
  return (
    <div className="w-full h-full flex items-center justify-between px-12 gap-8 max-sm:flex-col max-sm:py-3.5 max-sm:min-h-dvh max-sm:justify-center">
      ...
      <div className="max-w-md flex flex-col justify-between items-end space-y-6 ">
        <h1 className="text-7xl font-bold text-white max-sm:text-4xl max-sm:text-right">
          Mouhammad DIALLO
        </h1>
        <p className="text-5xl font-semibold text-[#CE9178] max-sm:text-right">
          Développeur web Full Stack
        </p>
        <p className="text-xl text-white italic max-sm:text-center">
          Every line of code is a puzzle piece. Let's assemble your perfect
          puzzle: fast, high-quality, and with minimal resource usage.
        </p>
        <button
